...
export async function POST(req: Request) {
	try {
		...
		const created = await prisma.position.create({ data: v.data });
		return NextResponse.json({ ok: true, data: created }, { status: 201 });
	} catch (err: any) {
		if (err?.code === "P2002") return NextResponse.json({ ok: false, message: "Position code already exists" }, { status: 409 });
...
export async function GET() {
  	const rows = await prisma.position.findMany({ orderBy: { code: "asc" } });
